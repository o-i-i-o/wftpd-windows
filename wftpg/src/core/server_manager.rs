...
impl ServerManager {
    ...
    pub fn is_service_installed(&self) -> bool {
        unsafe {
            let manager_result = OpenSCManagerW(
                None,
                None,
                SC_MANAGER_CONNECT,
            );
    ...
    pub fn is_service_running(&self) -> bool {
        unsafe {
            ...
            let service_name_wide: Vec<u16> = SERVICE_NAME.encode_utf16().chain(std::iter::once(0)).collect();
            let service = match OpenServiceW(manager, PCWSTR(service_name_wide.as_ptr()), SERVICE_QUERY_STATUS | SERVICE_START) {
                Ok(s) => s,
                Err(_) => {
                    let _ = CloseServiceHandle(manager);
                    return false;
    ...
    pub fn install_service(&self) -> Result<()> {
        unsafe {
            ...
            let service_name_wide: Vec<u16> = SERVICE_NAME.encode_utf16().chain(std::iter::once(0)).collect();
            let display_name_wide: Vec<u16> = "WFTPD Service".encode_utf16().chain(std::iter::once(0)).collect();
    ...
    pub fn uninstall_service(&self) -> Result<()> {
        unsafe {
            ...
            let service_name_wide: Vec<u16> = SERVICE_NAME.encode_utf16().chain(std::iter::once(0)).collect();
            let service = OpenServiceW(manager, PCWSTR(service_name_wide.as_ptr()), SERVICE_ALL_ACCESS)
                .context("无法打开服务")?;
