...
### FromConnection模式（推荐用于通配符绑定）

当服务器绑定0.0.0.0或::时，使用`PassiveHost::FromConnection`：

**工作原理**：
- libunftp在每次PASV命令时，自动获取TCP连接的本地端IP
- 无需手动选择，自动适配多IP地址环境
- 正确处理多网卡、多IP地址场景
