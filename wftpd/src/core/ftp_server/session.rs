...
use crate::core::path_utils::{
    PathResolveError, path_starts_with_ignore_case, resolve_directory_path, safe_resolve_path,
    to_ftp_path,
};
        ...
        match timeout_result {
            ...
            Ok(Ok(n)) => {
                ...
                while let Some(crlf_pos) = cmd_buffer.windows(2).position(|w| w == b"\r\n") {
                    ...
                    if !handle_command(&mut control_stream, &ftp_cmd, &mut state, &ctx).await? {
                        return Ok(());
                    }
                }
            }
        ...
        match timeout_result {
            ...
            Ok(Ok(n)) => {
                ...
                while let Some(pos) = cmd_buffer.iter().position(|&b| b == b'\n') {
                    ...
                    let should_continue =
                        handle_command(&mut control_stream, &cmd, &mut state, &ctx).await?;
    ...
    match cmd {
        ...
        PASV => {
            ...
            let passive_port = match state
                .passive_manager
                .try_bind_port(port_min, port_max, &bind_ip)
                .await
            {
                Ok(port) => port,
                Err(e) => {
            ...
            if ip_parts.len() != 4 {
                control_stream
                    .write_response(b"425 Invalid IP address format\r\n", "FTP response")
                    .await;
                return Ok(true);
            }
        ...
        HELP(cmd) => {
            if let Some(cmd) = cmd {
                let help_text = match cmd.to_uppercase().as_str() {
                    ...
                    "XPWD" => {
                        "214 XPWD: Print current working directory (deprecated, use PWD).\r\n"
                    }
                    "LIST" => {
                    ...
                    "EPSV" => {
                        "214 EPSV: Enter extended passive mode (supports IPv6, RFC 2428).\r\n"
                    }
                    "PORT" => {
                    ...
                    "STRU" => {
                        "214 STRU <structure>: Set file structure. F=File, R=Record, P=Page.\r\n"
                    }
                    "REST" => {
                    ...
                    "ABOR" => {
                        "214 ABOR: Abort current data transfer and close data connection.\r\n"
                    }
        ...
        RETR(filename) => {
            ...
            if let Some(filename) = filename {
                ...
                let starts_with_home = path_starts_with_ignore_case(&file_path, &state.home_dir)
                    || normalized_file_path_str
                        .to_lowercase()
                        .starts_with(&normalized_home_dir.to_lowercase());

                if !file_path.exists() || !file_path.is_file() || !starts_with_home {
                    tracing::warn!(
                        "RETR denied: path='{}', home='{}', exists={}, is_file={}, starts_with={}",
                        file_path.display(),
                        state.home_dir,
                        file_path.exists(),
                        file_path.is_file(),
                ...
                let is_ascii = state.transfer_mode == "ascii";
                let rate_limiter =
                    speed_limit_kbps.map(crate::core::rate_limiter::RateLimiter::new);
        ...
        STOR(filename) => {
            ...
            if let Some(filename) = filename {
                ...
                tracing::debug!(
                    "STOR: resolved='{}', normalized_home='{}', starts_with={}",
                    file_path.display(),
                    normalized_home_dir,
                    starts_with_home
                );
